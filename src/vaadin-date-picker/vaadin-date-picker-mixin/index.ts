...
import { Debouncer as PolymerDebouncer } from '@polymer/polymer/lib/utils/debounce';
import VaadinDatePickerHelper from '../vaadin-date-picker-helper';

const VaadinDatePickerMixin = subclass =>
class _VaadinDatePickerMixin extends PolymerMixinBehaviors([PolymerIronA11yKeysBehavior, PolymerIronResizableBehavior], subclass) {
  $:any;
  readonly:boolean;
  disabled:boolean;
  ...
  _handleDateChange(property, value, oldValue) {
    ...
    if (!VaadinDatePickerHelper._dateEquals(this[property], date)) {
      this[property] = date;
    }
  }
  ...
  _onOverlayOpened() {
    ...
    if (parsedInitialPosition ||
      VaadinDatePickerHelper._dateAllowed(initialPosition, this._minDate, this._maxDate)) {
      this.$.overlay.initialPosition = initialPosition;
    } else {
      this.$.overlay.initialPosition =
        VaadinDatePickerHelper._getClosestDate(initialPosition, [this._minDate, this._maxDate]);
    }
  ...
  checkValidity(value) {
    ...
    VaadinDatePickerHelper._dateAllowed(this._selectedDate, this._minDate, this._maxDate);
  ...
  _eventKey(e) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i];
      if (PolymerIronA11yKeysBehavior.keyboardEventMatchesKeys(e, k)) {
        return k;
      }
    }
  }
  ...
  _validateInput(date, min, max) {
    if (date && (min || max)) {
      this.invalid = !VaadinDatePickerHelper._dateAllowed(date, min, max);
    }
  }
  _onUserInput(e) {
    ...
    if (!this.opened) {
      ...
      if (this._ios) {
        ...
        this._debouncerRefit = PolymerDebouncer.debounce(this._debouncerRefit,
          PolymerAsync.timeOut.after(500), this.$.dropdown.refit);
      }
    }
  }
...
export default VaadinDatePickerMixin;
